...
| 服务档位 / Fast | `thread_settings_applied.thread_settings.service_tier` / `serviceTier` | 档位沿用到后续 `token_count`；`fast` 或 `priority` 标记为 Fast；`config.toml` 当前配置不回填历史 |
| 状态 | 正常请求级 `token_count` / `event_msg.error` | 前端只显示“成功 / 失败”；只有明确 `event_msg.error` 记为失败并展示 `error_reason` |
...
| 消耗金额 | `pricing.json` + token usage + service tier | 本地估算：标准 / Priority 分别使用官方模型费率；GPT-5.6 缺少 `cache_write_tokens` 时只给费用下限；不是官方账单 |
...
`buckets[]` 同时包含成功/失败数、成功率、缓存命中率、费用估算、平均响应耗时估算和平均首输出估算，用于周期统计视图。在 500 个 bucket 以内会补齐无请求周期：Token、请求数和费用为 `0`，成功率与延迟为 `null`，避免折线压缩真实时间空档。

## 系统维护事件排除规则

当 `last_token_usage.total_tokens > 0`，但 `input_tokens`、`cached_input_tokens`、`output_tokens` 全为 `0` 时，该事件没有请求级 usage 拆分。对真实 session 的前后事件审计确认了两类主要来源：

- `compacted` 后紧接的上下文压缩 token 统计；
- `turn_aborted` / `thread_rolled_back` 周边的状态恢复 token 统计。

这些事件不是 OpenAI 请求失败，也没有 `event_msg.error`。当前实现保留本地统计库的兼容读取能力，但从请求列表、成功/失败数、Token 汇总、费用和周期统计中排除，并通过 `diagnostics.excluded_system_token_count_records` 报告排除数量。
