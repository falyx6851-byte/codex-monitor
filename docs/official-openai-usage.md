...
当前 Codex session `last_token_usage` 可能只提供 `cached_input_tokens`，不提供 GPT-5.6 的 `cache_write_tokens`。这种情况下页面把未知部分暂按普通输入费率显示估算值；若未来 session 写出该字段，后端会从普通输入中扣除这部分并按缓存写入费率计算。

当前 Codex 模型元数据中的 GPT-5.6 `context_window` 为 `372,000`，`effective_context_window_percent` 为 `95`，所以 session 中可见 `model_context_window=353,400`。当前官方 GPT-5.6 价格表没有按上下文长度拆分费率；与 GPT-5.5 / GPT-5.4 的 `<272K context length` 标注不同，GPT-5.6 超过 `272K` 时不在本项目中应用长上下文倍率。
